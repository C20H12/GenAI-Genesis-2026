...
<template>
  <Card class="panel">
    ...
    <template #content>
      ...
      <template v-else>
        <InputText v-model="search" placeholder="Search by IP, destination, reason, status..." fluid />
        ...
        <DataTable :value="filteredEntries" paginator :rows="8" dataKey="id" stripedRows>
          ...
          <Column header="Reason">
            <template #body="slotProps">
              <button class="cell-link" @click="showFullText('Reason', slotProps.data.reason)">
                {{ truncateText(slotProps.data.reason, 100) }}
              </button>
            </template>
          </Column>
      ...
      <Dialog
        v-model:visible="isTextDialogOpen"
        :header="textDialogTitle"
        modal
      ...
      >
        ...
        <template #footer>
          ...
          <Button label="Delete" severity="danger" :loading="isDeleting" @click="deleteEntry" />
        </template>
      </Dialog>
    </template>
  </Card>
